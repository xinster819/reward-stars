...
**1. 正文偶尔出现 `=20` 碎片** —— `=20` 是 quoted-printable 对**空格**的编码，漏出来了。
- **不是数据错误**：所有积分数字都是真实计算结果，只是多了两处本该是空格的 `=20`。
- **根因**：`denomailer` 的 body QP 编码器会把 `=XX` 转义序列拆在软换行两侧，解码端遇到坏转义就原样吐出。
  出现位置取决于它在第几个字符处折行，**无法稳定规避**。
- **决定（2026-07-17，用户拍板）**：**接受现状**，不再打补丁。
- **想彻底解决**：换成 API 型邮件服务（Resend 等）——POST JSON，MIME 由服务端生成，
  这一整类编码 bug 连同下面第 2 条一起消失。改动点只有 `index.ts` 里 `sendEmail()` 那一段（约 15 行）。

**2. 邮件主题只能是英文** —— 见 `render.ts` 顶部注释。
`denomailer` 会把 header 里的 `=` 编成 `=3D`，而 RFC2047 encoded-word 必然含 `=` ⇒ 中文主题在这条链路上无解。
同样：换 API 型邮件服务即可恢复中文主题。

> 三个 denomailer bug 的完整调查记录见 git log：`ad4595d`（header 折行）、`c58b02b`（header 吃 `=`）。
