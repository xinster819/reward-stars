...
- **阶段 = Phase 4 Web/PWA 迁移，代码+云端验证全部完成。** iOS 工程冻结保留；App Store 物理项清单（SETUP §9）转为存档。**只剩两步**：① 用户授权 commit+push（Vercel 拉仓库的前置）；② Vercel 部署（Root=`web`，env 两条，SETUP §10 B）→ 可选域名 → 全家加主屏。**2026-07-03 用户授权后已 commit（`d868b0d`）并 push 到 `git@github.com:xinster819/reward-stars.git`。**
