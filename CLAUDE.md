...
9. **面向中国大陆用户的 Web 部署：自定义域名是前置必选项，不是事后备选。**
   - ❌（Phase 4 已发生）用户两次问"大陆能否访问"，只答风险评级、把域名当 fallback → `*.vercel.app` 移动网络果然被拦，多等一天买域名实名（用户明确不满）。
   - ✅ `*.github.io/vercel.app/pages.dev` 默认视为大陆不可用；"买域名（¥10–60/年，指向海外免备案）"与部署**并行**启动。用户重复追问的风险 = 关键决策项，回答必须含"现在就做的动作"。排查序：国内公共 DNS 解析 → 手机流量实测 → 自有域名直绑 Vercel → 套 Cloudflare。

10. **迁移/备份取数以数据库本体为准，快照文件先核新鲜度。**
   - ❌（Phase 4 已发生）iPad 迁移差点用"启动时写的 JSON 快照"——比 SQLite WAL 旧 4 天、少 20 条流水（快照只在启动时落盘，末次会话改动全在 WAL）。
   - ✅ 迁移前对比快照 mtime 与 store/WAL mtime，不一致直读数据库（SwiftData=Core Data SQLite：Z 表、UUID=16 字节 BLOB、日期=2001 纪元秒，sqlite3 打开自动并 WAL；签名过期的 App 可 `devicectl … appDataContainer` 沙盒直读）。
