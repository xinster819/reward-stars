...
### E. 每周积分周报邮件
> ✅ **已上线并实测通过（2026-07-17）**：Cron 面板显示 `Next run = 周一 08:00 (+0800)`；
> 手动触发实测 `sent:1`（Gmail SMTP 真发出）、重复触发 `skip_already:1`（幂等不重发）。

每周一早上 08:00（UTC+8）自动给家长邮箱发一封上一整周的孩子积分周报
（Supabase Edge Function + pg_cron + pg_net + **Gmail SMTP**）。
...
3. `supabase secrets set` 设 `GMAIL_USER / GMAIL_APP_PASSWORD / CRON_SECRET / REPORT_TZ / APP_URL`，并把 `project_url / cron_secret` 两条写入 Vault（两处 cron_secret 必须同值）。
4. `supabase functions deploy weekly-report --no-verify-jwt`，然后 `curl` 手动触发自测（见 README）。
...
> （周报邮件定时**已于 2026-07-17 实测通过**，见 §10.E。）
