...
2. 按 **`Cmd + R`** 运行。应看到孩子端首页（小明、总分、徽章…）。
3. 按 **`Cmd + U`** 跑测试，应全部通过。
...
> ⚠️ **务必用一个【全新的、干净的】公开仓库，不要对本 App 仓库开 Pages。**
> 本仓库的 git 历史里有真实孩子照片（`assets-src/20260621-150859.jpeg`，虽未打进 App）。免费账号的 GitHub Pages 需要**公开仓库**，对本仓库开 Pages 会把整段历史（含那张照片）公开。新建一个只放隐私页的干净仓库即可规避。

**待发布文件已备好**：`docs/privacy-page/index.html`（中英双语、自包含、移动端友好）。发布前把里面两处 `<联系邮箱>` / `<contact email>` 换成**你愿意公开**的联系邮箱（会随页面公开，建议别直接用主邮箱，可注册一个支持邮箱）。

**步骤（网页版）**：
1. GitHub 新建一个 **public** 仓库，例如 `reward-stars-privacy`。
2. 把本仓库的 `docs/privacy-page/index.html` 上传到新仓库**根目录**（文件名保持 `index.html`），并填好联系邮箱。
...
# 用编辑器把 index.html 里的 <联系邮箱>/<contact email> 改成你的邮箱
...
> 备选：也可用公开 **Gist** 或把 `PRIVACY.md` 作为 `index.md` 交给 Jekyll 渲染——但单文件 `index.html` + 干净仓库最省事、URL 最干净。同样**别用本 App 仓库**。
